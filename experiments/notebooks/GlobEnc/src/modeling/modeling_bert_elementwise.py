    ...
    def forward(
        ...
        if is_cross_attention and past_key_value is not None:
            ...
            if attribution_vectors is not None:
                decomposed_value_layer = self.transpose_for_scores_for_decomposed(self.value(attribution_vectors) - self.value.bias)
...
class BertNormOutput(nn.Module):  # This class was added by Goro Kobayashi
    ...
    def forward(self, hidden_states, attention_probs, value_layer, dense, LayerNorm, pre_ln_states, decomposed_value_layer, attribution_vectors, globenc_only):
        # Args:
        ...
        with torch.no_grad():
            ...
            if decomposed_value_layer is None:
                ...
                transformed_layer = torch.einsum('bhsqv,dhv->bhsqd', decomposed_value_layer, dense)  # V * W^o  (z=(qk)v)
                ...
                # Sum each weighted vectors αf(x) over all heads:
                # (batch, seq_length, seq_length, seq_length, all_head_size)
                summed_weighted_layer = weighted_layer.sum(dim=1)  # sum over heads
                if not globenc_only:
                    summed_weighted_norm = torch.norm(summed_weighted_layer, dim=-1)  # norm of ||Σαf(x)||
...
class BertEncoder(nn.Module):
    ...
    def forward(
        ...
        for i, layer_module in enumerate(self.layer):
            ...
            # added by Fayyaz / Modarressi
            if output_globenc:
                vectors = layer_outputs[2 + 4] if output_norms else layer_outputs[1]
