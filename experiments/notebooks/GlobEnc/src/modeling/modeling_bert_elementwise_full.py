...
class BertAttention(nn.Module):
    ...
    def forward(
        ...
        attention_output = self.output(
            self_outputs[0],
            hidden_states,
            output_norms=output_globenc,  # added by Goro Kobayashi (Edited by Fayyaz / Modarressi)
        )
...
class BertIntermediate(nn.Module):
    ...
    def forward(self, hidden_states: torch.Tensor, output_globenc: Optional[bool] = False) -> torch.Tensor:
...
class BertOutput(nn.Module):
    ...
    def forward(self, hidden_states: torch.Tensor, input_tensor: torch.Tensor, output_globenc: Optional[bool] = False):
        hidden_states = self.dense(hidden_states)
        hidden_states = self.dropout(hidden_states)
        # hidden_states = self.LayerNorm(hidden_states + input_tensor)
...
class BertLayer(nn.Module):
    ...
    def bias_decomposer(self, bias, attribution_vectors):
        # Deconposes the input bias based on similarity to the attribution vectors
        # Args:
        ...
        # abs_similarity = torch.abs(self.similarity_fn(attribution_vectors, bias))
        abs_similarity = torch.abs(torch.einsum("bskd,d->bsk", attribution_vectors, bias))
        weights = abs_similarity / (abs_similarity.sum(dim=-1, keepdim=True) + 1e-12)
    ...
    def ln_decomposer(self, attribution_vectors, pre_ln_states, gamma, beta, eps):
        ...
        return post_ln_layer + self.bias_decomposer(beta, post_ln_layer)
    ...
    def gelu_decomposition(self, attribution_vectors, intermediate_hidden_states, intermediate_output):
        ...
        abs_similarity = torch.abs(torch.einsum("bskl,bsl->bsk", mx, b))
        weights = abs_similarity / (abs_similarity.sum(dim=-1, keepdim=True) + 1e-12)
    ...
    def ffn_decomposer(self, attribution_vectors, intermediate_hidden_states, intermediate_output):
        ...
        post_first_layer = post_first_layer + self.bias_decomposer(self.intermediate.dense.bias, post_first_layer)
        # mask_for_gelu_approx = (intermediate_hidden_states > 0) * 1.5
        # masked_first_layer = torch.einsum("bskl, bsl->bskl", post_first_layer, mask_for_gelu_approx)
        # masked_first_layer = post_first_layer * mask_for_gelu_approx.unsqueeze(dim=-2)
        masked_first_layer = self.gelu_decomposition(post_first_layer, intermediate_hidden_states, intermediate_output)
        post_second_layer = torch.einsum("bskl, dl->bskd", masked_first_layer, self.output.dense.weight)
        post_second_layer = post_second_layer + self.bias_decomposer(self.output.dense.bias, post_second_layer)
    ...
    def forward(
        ...
        # Added by Fayyaz / Modarressi
        # -------------------------------
        intermediate_output, pre_act_hidden_states = self.intermediate(attention_output, output_globenc=output_globenc)
        layer_output, pre_ln2_states = self.output(intermediate_output, attention_output, output_globenc=output_globenc)
        if output_globenc:
            ...
            if decomposed_value_layer is None:
                ...
                accumulated_bias = torch.matmul(self.attention.self.value.bias, self.attention.output.dense.weight) + self.attention.output.dense.bias

            residual_weighted_layer = residual_weighted_layer + self.bias_decomposer(accumulated_bias, residual_weighted_layer)
            ...
            post_ffn_layer = self.ffn_decomposer(
...
class BertEncoder(nn.Module):
    ...
    def forward(
            ...
            output_globenc_vectors: Optional[bool] = None  # added by Fayyaz / Modarressi
        ...
        attribution_vectors = None # added by Fayyaz / Modarressi
        all_attribution_vectors = ()
        ...
        for i, layer_module in enumerate(self.layer):
            ...
            # added by Fayyaz / Modarressi
            if output_globenc:
                vectors = layer_outputs[1]
                attribution_vectors = vectors
                # # vectors = vectors * torch.exp(attention_mask).view((-1, attention_mask.shape[-1], 1, 1))
                # if globenc_attributions is None:
                #     # globenc_attributions = vectors
                #     globenc_attributions = torch.einsum("ijd,ijkd->")
                #     # globenc_attributions = globenc_attributions / globenc_attributions.sum(dim=(1,2))
                # else:
                #     globenc_attributions = torch.einsum("ijkd,ikmd->ijmd", vectors, globenc_attributions)
                #     # globenc_attributions = globenc_attributions / globenc_attributions.sum(dim=(1,2))
            
            if output_globenc_vectors:
                all_attribution_vectors = all_attribution_vectors + (attribution_vectors,)
        
        if output_globenc:
            globenc_attributions = torch.norm(attribution_vectors, dim=-1)

        if not output_globenc_vectors:
        ...
        if not return_dict:
            return tuple(
                ...
                for v in [
                    ...
                    all_attribution_vectors    # Added by Fayyaz / Modarressi
                ]
                if v is not None
            )
...
class BertModel(BertPreTrainedModel):
    ...
    def forward(
        ...
        encoder_outputs = self.encoder(
            ...
            output_globenc=output_globenc, # added by Fayyaz / Modarressi
            output_globenc_vectors=output_globenc_vectors # added by Fayyaz / Modarressi
        )
...
class BertForSequenceClassification(BertPreTrainedModel):
    ...
    def forward(
        ...
        outputs = self.bert(
            ...
            return_dict=return_dict,
            output_globenc=output_globenc,
            output_globenc_vectors=output_globenc_vectors
        )
